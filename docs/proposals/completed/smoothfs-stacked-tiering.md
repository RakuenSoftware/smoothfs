...
**Status:** Complete for v1. Phase 0 contract, Phase 1 scaffold, and all Phase 2 work through Phase 2.7 (heat + movement state machine, per-fd cutover reissue, writable-fd movement, SmoothKernel extraction, `vma_set_file` mmap pivot, auto-discovery + revoke, live E2E validation, and kernel placement-log replay + crash-recovery parity) have landed. Phase 3 functional compatibility validation for pool filesystems (`xfs`, `ext4`, `btrfs`) has now landed as well, and the in-tree non-functional benchmark harness is in place. Phase 4 (NFS, through cthon04 + connectable filehandles) and Phase 5 (SMB, through smbtorture baseline + the full Samba VFS module — skeleton, lease pin, fanotify lease-break watcher, and SMB FileId) have both landed. Phase 6 (iSCSI) has also landed — O_DIRECT conformance (6.0), end-to-end LIO file-backed LUN round-trip over smoothfs (6.1), `trusted.smoothfs.lun` pin contract (6.2), target restart / reconnect correctness across `targetctl save / clear / restore` + `iscsid` bounce (6.3), and tierd-side file-backed LUN orchestration that auto-pins `PIN_LUN` on create and clears on destroy (6.5). Individual tier backends remain a separate matrix (`zfs`, `mdadm`, `btrfs`). Phase 7 (appliance integration + kernel update path + signed-module pipeline) has also landed: three apt-installable debs (`smoothfs-dkms`, `smoothfs-samba-vfs`, `tierd`), DKMS-signed modules with an operator MOK enrollment helper, a kernel-upgrade regression harness, a tierd-ui surface for pool + LUN lifecycle including a quiesce/reconcile + movement-log observability page, and a published support matrix + operator runbook at `docs/smoothfs-{support-matrix,operator-runbook}.md`. Active-LUN movement is now implemented as rollout-gated Phase 8 work tracked in [`smoothfs-active-lun-movement.md`](../completed/smoothfs-active-lun-movement.md).
...
Lift the "LUN backing files are pinned by default, and only an administrative target quiesce clears the pin" ruling from Phase 0 §iSCSI so tierd can move a live LUN between tiers. This deferred work now lives in [`smoothfs-active-lun-movement.md`](../completed/smoothfs-active-lun-movement.md). Phase 6.5's `CreateFileBackedTarget` / `DestroyFileBackedTarget` lifecycle and Phase 6.2's `trusted.smoothfs.lun` pin contract are the only smoothfs-side prerequisites; everything else in Phase 8 is new.
