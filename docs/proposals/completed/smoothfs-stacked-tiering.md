...
Phase 8 (active-LUN movement model — operator quiesce → clear `PIN_LUN` → journaled move → re-pin) was originally the open "6.4 — active-LUN movement model not relaxed until proven in test" carry-over inside Phase 6. With 6.5 landing the tierd-side pin lifecycle, Phase 6 closes cleanly; the remaining work is big enough (operator quiesce protocol, journaled move with a new pre-cutover gate, crash recovery, correctness + fault-injection tests) to stand on its own. It is now tracked by [`smoothfs-active-lun-movement.md`](../pending/smoothfs-active-lun-movement.md) and remains gated by Phase 6 soaking in production.
...
Lift the "LUN backing files are pinned by default, and only an administrative target quiesce clears the pin" ruling from Phase 0 §iSCSI so tierd can move a live LUN between tiers. This deferred work now lives in [`smoothfs-active-lun-movement.md`](../pending/smoothfs-active-lun-movement.md). Phase 6.5's `CreateFileBackedTarget` / `DestroyFileBackedTarget` lifecycle and Phase 6.2's `trusted.smoothfs.lun` pin contract are the only smoothfs-side prerequisites; everything else in Phase 8 is new.
