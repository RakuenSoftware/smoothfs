...
| Range-level staging | Supported | Buffered niet-truncating writes naar niet-gepinde cold-tier reguliere files kunnen gewijzigde ranges op de snelste tier stage; oude bytes worden later correct via range read-merge gelezen. Zodra ranges staged zijn, wordt direct I/O en mmap geweigerd zodat bypass niet meer mogelijk is. Range-metadata wordt naar sidecars op de snelste tier geschreven en op remount gereplayed. |
| Range-level drain terug naar HDD | Supported | Als SmoothNAS `write_staging_drain_active_tier_mask` met bronbit zet, kopieert smoothfs in-memory of door replay herstelde staged ranges terug naar die bron-lower file, fsynct, wist staged-range state, verwijdert de snelle sidecar en wist de recovery sidecar. SmoothNAS moet bits alleen zetten na externe activiteit. |
