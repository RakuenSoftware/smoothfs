...
|---|---|---|
| Sysfs status/control | Supported | `/sys/fs/smoothfs/<uuid>/write_staging_supported`, `write_staging_enabled`, `write_staging_full_pct`, `staged_bytes`, `staged_rehome_bytes`, `range_staged_bytes`, `range_staged_writes`, `staged_rehomes_total`, `staged_rehomes_pending`, `write_staging_drainable_rehomes`, `write_staging_drain_pressure`, `write_staging_drainable_tier_mask`, `oldest_staged_write_at`, `last_drain_at`, `last_drain_reason`, `write_staging_drain_active_tier_mask`, `range_staging_recovery_supported`, `range_staging_recovered_bytes`, `range_staging_recovered_writes`, `range_staging_recovery_pending`, `recovered_range_tier_mask`, `oldest_recovered_write_at`, `last_recovery_at`, `last_recovery_reason` |
...
| Range-level staging | Supported | Buffered non-truncating writes to unpinned cold-tier regular files can stage changed ranges on the fastest tier while preserving old-byte reads through range read-merge. Once ranges are staged, direct I/O and mmap are refused so callers cannot bypass the merge layer. Range metadata is persisted to fastest-tier sidecars and replayed on remount. |
| Range-level drain back to HDD | Supported | When SmoothNAS writes `write_staging_drain_active_tier_mask` with the source tier bit set, smoothfs copies in-memory or replay-recovered staged ranges back to that source lower file, fsyncs, clears staged-range state, removes the fastest-tier sidecar, and clears the recovery sidecar. SmoothNAS must only set non-fast bits after external activity has already woken the tier. |
