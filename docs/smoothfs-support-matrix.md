...
|---|---|---|
| Sysfs status/control | Supported | `/sys/fs/smoothfs/<uuid>/write_staging_supported`, `write_staging_enabled`, `write_staging_full_pct`, `staged_bytes`, `staged_rehomes_total`, `write_staging_drainable_rehomes`, `write_staging_drain_pressure`, `write_staging_drainable_tier_mask`, `oldest_staged_write_at`, `last_drain_at`, `last_drain_reason`, `write_staging_drain_active_tier_mask` |
