...
| Range-level staging | Partial | Buffered non-truncating writes to unpinned cold-tier regular files can stage changed ranges on the fastest tier while preserving old-byte reads through range read-merge. Direct I/O, mmap, persistence replay, and drain-back remain pending. |
| Range-level drain back to HDD | Pending | smoothfs exposes `write_staging_drain_active_tier_mask`, `write_staging_drainable_tier_mask`, `write_staging_drainable_rehomes`, and `write_staging_drain_pressure` so future range-level drain code can target only tiers that SmoothNAS has observed active due to external activity, or report fullness pressure without touching standby disks. |
